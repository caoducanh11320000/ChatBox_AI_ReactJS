...
// // ✅ Hàm streaming phản hồi theo kiểu giống hình ảnh bạn cung cấp
// async function streamResponse(messages, updateChat) {
//     const response = await client.chat.completions.create({
//         model: "meta-llama/Llama-3-70b-chat-hf",
//         messages: messages,
...
//     let botMessage = { role: "assistant", content: "" };
//     updateChat(botMessage); // ✅ Hiển thị bot ngay lập tức với nội dung rỗng

//     for await (const chunk of response) {
//         const text = chunk.choices[0]?.delta?.content || "";
//         botMessage.content += text;
//         updateChat({ ...botMessage }); // ✅ Cập nhật từng chữ lên UI
//     }
// }
...
//         toast.success("API Key đã được cập nhật!");
//     };
...
//         // ✅ Thêm tin nhắn bot rỗng ngay lập tức
//         let botMessage = { role: "assistant", content: "" };
//         setChatHistory(prevChat => [...prevChat, botMessage]);

//         // ✅ Gọi streaming API để cập nhật từng chữ theo thời gian thực
//         await streamResponse([...chatHistory, userMessage], (updatedMessage) => {
//             setChatHistory(prevChat => [...prevChat.slice(0, -1), updatedMessage]);
//         });
...
//                 <form className="flex" onSubmit={submitAPIKey}>
//                     <input type="text" placeholder="Nhập API Key..." onChange={(e) => set_apikey(e.target.value)} className="flex-grow p-2 rounded-l border" />
//                     <button type="submit" className="bg-blue-500 text-white px-4 py-2 rounded-r">Enter API Key</button>
//                 </form>

//                 <form className="flex" onSubmit={submitForm}>
//                     <input type="text" placeholder="Tin nhắn của bạn..." value={message} onChange={(e) => setMessage(e.target.value)} className="flex-grow p-2 rounded-l border" />
//                     <button type="submit" className="bg-blue-500 text-white px-4 py-2 rounded-r">Gửi</button>
//                 </form>

//                 <div className="flex-grow overflow-y-auto mt-4 bg-white rounded shadow p-4">
//                     {chatHistory.map((chatMessage, i) => <div key={i}>{chatMessage.content}</div>)}
//                 </div>
//             </div>
//         </div>
//     );
